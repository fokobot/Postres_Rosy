...
export default {
  ...
  mixins: [getErrors],
  created() {
    let id = this.$route.params.producto
    this.$store.dispatch('productos/fetch', id)
    Vue.set(this.producto, 'id', id)
  },
  computed: {
    ...mapGetters({
      getProductoByID: 'productos/getProductoByID',
  ...
  methods: {
    ...
    loading(isLoading, wasLoading) {
      if (!isLoading && wasLoading) {
        Vue.set(this, 'producto', this.getProductoByID(this.producto.id))
      }
    }
  }
}
